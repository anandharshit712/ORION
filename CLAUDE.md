...
- Dashboard sections: `overview`, `scenarios`, `runs`, `models`, `batches` render real data; `compare` and `settings` still show the `ComingSoon` panel. `SECTIONS` in `DashboardPage.jsx` maps a view key to its component, and `NAV_ITEMS` in `Sidebar.jsx` carries a `ready` flag — unready entries stay visible with a `soon` marker but are **disabled**. **The two must agree**: `DashboardSections.test.jsx` asserts `ready === Boolean(SECTIONS[key])`, so flipping one without the other fails the suite rather than dropping someone on an empty page. `compare` needs a regression endpoint (`RegressionDetector` is CLI-only); `settings` needs the org/API-key views.
- **Every data view uses `DataStates`** (`components/dashboard/DataStates.jsx`) for the loading/empty/error trio `docs/UI_DESIGN.md` §9.12 requires. Don't hand-roll them per section: the one that gets skipped is always the error state, and an empty table rendered on a 500 reads as "no data". `useApiData` (`hooks/useApiData.js`) is the fetch side — it ignores responses that arrive after unmount.
- **Six pages exist but are not routed**: `BatchPage`, `ComparePage`, `ModelsPage`, `RunPage`, `SearchPage`, `SettingsPage`. Each declares a `/dashboard/...` route in its header comment that `App.jsx` does not define, and each is presentational with data wiring explicitly out of scope. The dashboard navigates by `setView` state instead, so those files are currently unreachable. Decide one pattern before building on either — don't add a seventh unrouted page.
