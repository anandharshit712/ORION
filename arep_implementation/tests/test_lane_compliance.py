...
def _snapshot(offset: float, *, lane_valid: bool = True, velocity: float = 10.0):
    return EgoSnapshot(
        sim_time=0.0, x=0.0, y=offset, heading=0.0,
...
def test_signed_offset_is_negative_to_the_left_of_travel():
    ...
    lane = LaneInfo("l0", [Vector2D(0, 0), Vector2D(100, 0)], width=LANE_WIDTH,
                    speed_limit=30.0)
...
def test_the_boundary_is_where_the_body_edge_touches_the_line():
    ...
    assert ComplianceMetrics().compute(
        _record([_snapshot(just_inside)])).lane_compliance_fraction == 1.0
    assert ComplianceMetrics().compute(
        _record([_snapshot(just_outside)])).lane_compliance_fraction == 0.0


def test_partial_drift_gives_a_partial_fraction():
    """The acceptance criterion: drifting out of lane scores below 1.0."""
    snapshots = [_snapshot(0.0) for _ in range(30)] + [_snapshot(1.6) for _ in range(20)]
...
def test_a_scenario_with_no_lane_geometry_cannot_fail_lane_keeping():
    ...
    assert ComplianceMetrics().compute(
        _record(snapshots)).lane_compliance_fraction == 1.0
